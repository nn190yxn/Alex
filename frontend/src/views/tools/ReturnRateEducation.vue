    ...
    <template #inputs>
      ...
      <div class="roi-result" v-if="result && !result.error">
        ...
        <div class="result-details">
          <div class="detail-item"><span>投入金额</span><span class="numeral">¥{{ result.investment }}</span></div>
          <div class="detail-item"><span>产出金额</span><span class="numeral">¥{{ result.output }}</span></div>
...
const form = reactive({ investment: null, output: null, classType: '小班', subjectType: 'K12学科' })
...
async function handleSubmit() {
  ...
  try {
    const data = await generateTool('return-rate-education', { ...form, return: form.output })
