    ...
    <template #inputs>
      <div class="form-row">
        <div class="form-group">
          ...
          <input v-model.number="form.investment" type="number" class="form-input" placeholder="开店/新项目总投资" min="0" />
        </div>
        <div class="form-group">
    ...
    <template #result>
      <div class="payback-result" v-if="result && !result.error">
        <div class="result-main">
          ...
          <div class="result-sub">约 {{ result.years }} 年</div>
        </div>
        <div class="result-details">
          ...
          <div class="detail-item">
            <span>月净利</span>
            <span class="numeral">¥{{ form.monthlyNetProfit }}</span>
          </div>
          <div class="detail-item">
            <span>年化收益率</span>
            <span class="numeral">{{ result.annualReturn }}%</span>
          </div>
        </div>
...
function handleSubmit() {
  if (!form.investment || !form.monthlyNetProfit || form.investment <= 0 || form.monthlyNetProfit <= 0) {
    result.value = { error: '请输入有效的总投资额和月净利' }
    return
  }
  ...
  let statusText = '一般'
  let suggestion = ''
  let reference = '餐饮12-24个月为正常，>36个月风险大'

  if (months <= 12) {
    status = 'success'
    statusText = '优秀 — 回本很快'
    suggestion = '回本周期很短，投资价值高！建议加快扩张步伐，复制成功模式。'
  } else if (months <= 24) {
    status = 'success'
    statusText = '正常 — 回本周期合理'
    suggestion = '在正常回本范围内。关注月净利的稳定性，确保能持续达到预期利润。'
  } else if (months <= 36) {
    status = 'warning'
    statusText = '偏长 — 回本压力大'
    suggestion = '回本周期偏长。建议：1）提升月净利（增加营收或降低成本）；2）控制初始投资额；3）评估项目是否值得继续。'
  } else {
    status = 'danger'
    statusText = '风险大 — 回本遥遥无期'
    suggestion = '回本周期过长，投资风险极大！建议：1）重新评估项目可行性；2）降低投资规模；3）考虑是否有更好的投资方向。'
  }
