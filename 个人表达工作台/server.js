...
function patchSkill(user, rid, patch) {
  const own = listResources(user, 'skill').find(item => item.id === rid);
  // 内置写法包只有一份全局记录，归首个创建它的用户所有。别的用户也能看到并切换它，
  // 只按当前用户查会误报 404（写法包页的启用开关就点不动）。这里回退到全局记录。
  const row = own ? null : skillRowById(rid);
  if (!own && !row) fail('resource not found', 404);
  const current = own || { id: row.id, ...JSON.parse(row.data) };
  const recordOwner = own ? user : row.owner;
  const next = { ...patch };
  ...
  return updateResource(recordOwner, 'skill', rid, next);
}
