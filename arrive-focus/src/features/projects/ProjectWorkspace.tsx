    ...
    <div className="projects-workspace">
      ...
      <section className="project-detail">
        ...
        {tab === "overview" ? <ProjectOverview summary={selectedDetail?.summary ?? selected} saving={saving} onStatusChange={changeStatus} /> : null}
         {tab === "tasks" ? <ProjectTasks detail={selectedDetail} taskBusy={taskBusyId !== null} onAddTask={() => onAddTask?.(selected.project)} onToggleTask={toggleTask} onRemoveTask={removeTask} /> : null}
...
function ProjectTasks({ detail, taskBusy, onAddTask, onToggleTask, onRemoveTask }: { detail: ProjectDetail | null; taskBusy: boolean; onAddTask: () => void; onToggleTask: (task: TaskRecord) => void; onRemoveTask: (task: TaskRecord) => void }) {
  const { t } = useI18n();
  const tasks = detail?.tasks ?? [];
  const pending = tasks.filter((task) => task.status === "pending").length;
  return <Panel className="project-tab-panel"><div className="section-heading"><div><span className="eyebrow">{t("project.activeTasks")}</span><h2>{t("project.pendingCount", { count: pending })}</h2></div><Button tone="primary" disabled={taskBusy} onClick={onAddTask}>{t("project.addTask")}</Button></div>{tasks.length === 0 ? <p>{t("project.noTasks")}</p> : tasks.filter((task) => task.status !== "removed").map((task) => <article className="project-task-item" key={task.id}><button className={`task-check ${task.status === "completed" ? "done" : ""}`} aria-label={t(task.status === "completed" ? "task.restoreLabel" : "task.completeLabel", { title: task.title })} disabled={taskBusy} onClick={() => onToggleTask(task)} /><div><strong>{task.title}</strong><small>{task.scheduledDate ?? t("project.unscheduled")}</small></div><Badge>{t(task.status === "completed" ? "task.state.completed" : "task.state.normal")}</Badge><Button className="project-task-item__remove" type="button" tone="ghost" disabled={taskBusy} aria-label={t("task.removeLabel", { title: task.title })} onClick={() => onRemoveTask(task)}>{t("task.remove")}</Button></article>)}</Panel>;
}
