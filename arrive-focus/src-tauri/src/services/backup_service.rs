...
use crate::{
    domain::backup::{
        ...
        CURRENT_BACKUP_FORMAT_VERSION, MAX_BACKUP_BYTES,
    },
    repositories::{
...
impl<'a> BackupService<'a> {
    ...
    pub fn parse_json(input: &str) -> Result<ValidatedBackup, DomainError> {
        ...
        let value: serde_json::Value = serde_json::from_str(input).map_err(format_error)?;
...
mod tests {
    ...
    fn parser_reports_format_and_version_errors_separately() {
        ...
        assert_eq!(
            BackupService::parse_json(r#"{"formatVersion":2}"#)
                .unwrap_err()
                .code,
            "BACKUP_VERSION_UNSUPPORTED"
        );
        ...
        target
            .write(|transaction| {
                transaction.execute_batch(
                    "CREATE TRIGGER reject_restored_project
                     BEFORE INSERT ON projects
                     WHEN NEW.id = 'project-1'
                     BEGIN
