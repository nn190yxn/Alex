...
use crate::{
    ...
    repositories::{analysis_repository::{AnalysisEvent, AnalysisRepository}, database::Database},
    DomainError,
};
...
impl<'a> AnalysisService<'a> {
    ...
    pub fn get_task_breakdown(&self, query: AnalysisQuery) -> Result<AnalysisSummary, DomainError> {
        ...
        if starts_on > ends_on {
            return Err(error("ANALYSIS_RANGE_INVALID", "startsOn must be on or before endsOn", None));
        }
        let timezone = query.timezone.parse::<Tz>().map_err(|_| {
            error("ANALYSIS_TIMEZONE_INVALID", "timezone must be a valid IANA timezone", Some("timezone"))
        })?;
        let after_end = ends_on.checked_add_signed(Duration::days(1)).ok_or_else(|| {
            error("ANALYSIS_DATE_INVALID", "date range exceeds supported dates", None)
        })?;
        ...
        let mut events = repository.list_completed_events(&utc_start.to_rfc3339(), &utc_end.to_rfc3339())?;
        events.extend(repository.list_focus_events(&utc_start.to_rfc3339(), &utc_end.to_rfc3339())?);
...
fn build_summary(query: &AnalysisQuery, events: Vec<AnalysisEvent>) -> AnalysisSummary {
    ...
    for event in events {
        let row = rows.entry(event.task_id.clone()).or_insert_with(|| TaskAnalysisRow {
        ...
        if let Some(instance_id) = event.task_instance_id {
            instance_ids.entry(event.task_id).or_default().insert(instance_id);
        }
    }
...
fn compare_rows(left: &TaskAnalysisRow, right: &TaskAnalysisRow, sort: AnalysisSort) -> Ordering {
    let primary = match sort {
        ...
        AnalysisSort::EffectiveSessionCount => right.effective_session_count.cmp(&left.effective_session_count),
        AnalysisSort::Title => left.title.cmp(&right.title),
    };
    primary.then_with(|| left.title.cmp(&right.title)).then_with(|| left.task_id.cmp(&right.task_id))
}

fn parse_date(value: &str, field: &str) -> Result<NaiveDate, DomainError> {
    NaiveDate::parse_from_str(value, "%Y-%m-%d").map_err(|_| {
        error("ANALYSIS_DATE_INVALID", "date must use YYYY-MM-DD", Some(field))
    })
}
...
fn local_day_start(date: NaiveDate, timezone: Tz) -> Result<DateTime<Utc>, DomainError> {
    ...
    for minute in 0..=1_440 {
        ...
        match timezone.from_local_datetime(&local) {
            ...
            LocalResult::Ambiguous(first, second) => return Ok(first.min(second).with_timezone(&Utc)),
            LocalResult::None => {}
        }
    }
    Err(error("ANALYSIS_TIMEZONE_INVALID", "timezone has no valid instant for a calendar boundary", Some("timezone")))
}

fn error(code: &str, message: &str, field: Option<&str>) -> DomainError {
    DomainError { code: code.into(), message: message.into(), field: field.map(str::to_string) }
}
...
mod tests {
    ...
    fn event(task_id: &str, instance_id: Option<&str>, completed_at: Option<&str>, seconds: u64, effective: bool, cancelled: bool) -> AnalysisEvent {
        AnalysisEvent {
    ...
    fn aggregates_events_and_keeps_cancelled_sessions_out_of_effective_metrics() {
        let query = AnalysisQuery { starts_on: "2026-07-20".into(), ends_on: "2026-07-21".into(), timezone: "UTC".into(), sort: AnalysisSort::CompletedCount };
        let result = build_summary(&query, vec![
            event("task-1", Some("instance-1"), Some("2026-07-20T10:00:00Z"), 0, false, false),
    ...
    fn validates_date_order_and_timezone() {
        let service_query = AnalysisQuery { starts_on: "2026-07-22".into(), ends_on: "2026-07-20".into(), timezone: "UTC".into(), sort: AnalysisSort::Title };
        assert_eq!(parse_date(&service_query.starts_on, "startsOn").unwrap(), NaiveDate::from_ymd_opt(2026, 7, 22).unwrap());
        assert_eq!("ANALYSIS_DATE_INVALID", parse_date("bad", "startsOn").unwrap_err().code);
