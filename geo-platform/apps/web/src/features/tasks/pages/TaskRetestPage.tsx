...
import type { OptimizationTask, OptimizationTaskInput, OptimizationTaskUpdateInput, RetestPlanInput, RetestResultInput, TaskBoardDashboard } from '@geo-platform/shared-types';
  ...
  const resultMutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response.success) {
        ...
        void invalidate();
        void messageApi.success('再次测试结果已保存');
      } else {
        void messageApi.error(response.error.message);
      }
    }
    ...
    <Space direction="vertical" size={16} className="page-stack">
      ...
      <Card title="任务跟进" extra={<Button type="primary" onClick={() => setCreateOpen(true)}>新建任务</Button>}>
        <Typography.Paragraph>
          跟踪每个优化动作由谁负责、做到哪一步、关联哪条内容，以及下一次 AI 测试结果。
        </Typography.Paragraph>
        <Space size={24} wrap>
          ...
          { title: '再次测试记录', render: (_, record) => record.retestRecords.length },
          {
            ...
            render: (_, record) => (
              <Space>
                ...
                <Button size="small" onClick={() => setRetestTask(record)}>安排再次测试</Button>
      ...
      <Modal title="录入再次测试结果" open={Boolean(resultTask)} okText="保存" cancelText="取消" onCancel={() => setResultTask(undefined)} onOk={() => resultForm.submit()} confirmLoading={resultMutation.isPending}>
        <Form form={resultForm} layout="vertical" initialValues={{ targetScore: resultTask?.retestRecords[0]?.targetScore ?? 80 }} onFinish={(values) => {
          ...
          <Form.Item name="targetScore" label="目标分"><InputNumber min={0} max={100} style={{ width: '100%' }} /></Form.Item>
          <Form.Item name="notes" label="测试结论"><Input.TextArea rows={3} /></Form.Item>
        </Form>
      </Modal>
    </Space>
  );
...
const taskTypeOptions = [
  ...
  { value: 'monitoring_issue', label: 'AI 测试问题' },
