...
个人表达工作台预览工程位于 `当前工作区/personal-expression-preview/`，是 Node.js 内置 HTTP 服务加静态 HTML 的 MVP。当前已验证 SQLite 持久化、演示模式异步改写、事实分析、样本与规则管理、版本确认/回滚、Markdown 文档导出、Skill/Prompt 导出、团队审核、权限和审计。PDF、DOCX、网页解析以及真实 LLM 评估和插件连接仍未完成。
