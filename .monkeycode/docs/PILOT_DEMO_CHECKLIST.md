...
本清单用于第五阶段试点客户演示。默认内存仓储内置一套最小 demo 数据；Prisma 演示环境可通过 `当前工作区/geo-platform/apps/api/prisma/seed.js` 写入稳定 demo 数据，覆盖品牌、监测、内容、发布、任务、报告和顾问记录，支持按固定路径完成演示、验收和反馈回填。
