...
本文档用于多品牌 GEO 管理平台生产试运行部署。当前工程位于 `当前工作区/geo-platform/`，采用 npm workspaces 组织 API、Web 和共享类型。
...
生产启动方式由部署平台负责进程管理。API 服务入口位于 `当前工作区/geo-platform/apps/api/dist/main.js`，Web 构建产物位于 `当前工作区/geo-platform/apps/web/dist/`。
