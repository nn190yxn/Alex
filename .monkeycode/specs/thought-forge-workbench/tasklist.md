...
- [x] 1.1 建立 `thought-forge/` 工程，配置 Tauri 2、React 19、TypeScript 严格模式、Vite 7、Vitest 与 pnpm 10
...
遗留说明：UI 规范中资产马赛克的「面积映射使用频次」暂缓——当前数据模型不记录 Skill 使用次数，改为等面积网格、颜色映射分类；待使用频次进入采集口径后再补面积映射。桌面壳仍未编译，命令层靠解析与前端契约验证。
...
- 在外壳内接入真实 OS 采集（剪贴板序列号、前台窗口钩子、文件 notify 监听）、主动搜集检索与真实模型平台，补齐端到端联调。
