...
- **内测可用性**: 用户可以按真实业务目标完成测试、分析、内容生成、发布准备和复测安排，并能理解每个结果的业务含义。
- **测试痕迹**: 面向用户界面中出现的 demo、mock、user_demo、内部 ID、英文 code、伪链接和占位内容。
...
**User Story:** AS 品牌方内测用户, I want 页面看起来像正式产品, so that 我不会被 demo、mock 和内部代号干扰。
...
3. WHEN 页面展示手动或演示路径, the system SHALL 使用人工录入、演示 AI 等中文名称。
...
1. WHEN 用户启动自动化运营, the system SHALL 引导用户完成测试问题确认、AI 测试、回答分析、内容生成、平台改写、发布建议和复测建议。
...
**User Story:** AS 内测用户, I want 知道当前功能能做到哪一步, so that 我可以有效测试而不是误以为系统坏了。

#### Acceptance Criteria

1. WHEN 功能依赖外部 API Key, the system SHALL 使用业务语言说明“补充平台密钥后可自动测试”。
2. WHEN 浏览器辅助测试需要登录或人工确认, the system SHALL 说明需要用户完成的平台动作。
...
1. 第一批修复优先处理内容草稿深度、测试痕迹、平台名中文化和伪链接。
2. 第二批修复补齐竞品地图发现、发布中心人工发布闭环和内测反馈入口。
3. 第三批修复完善浏览器辅助测试、文档解析和真实平台发布自动化。
