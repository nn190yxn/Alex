...
本设计将现有多品牌 GEO 平台从后台运营工具扩展为品牌方可自助使用的完整增长流程。系统围绕“上传资料、选择问法、连接平台、开始测试、查看建议、执行优化、复测增长”组织功能，复用现有品牌知识库、AI 平台配置、监测运行、回答分析、内容策略、内容生成、发布中心和任务复测能力，新增资料导入解析、测试主题与问法候选、测试计划、浏览器辅助连接、业务化结果解释和 GEO 增长优化计划边界。
...
前端保留当前 React Router、品牌上下文和后台布局，在品牌工作区、AI 监测、内容生成、发布中心和任务复测页面之间增加连续流程入口。后端继续通过 NestJS controller、repository port、Prisma schema 和 shared-types 维护前后端契约。真实 AI API 调用继续走 Adapter registry；浏览器辅助测试作为新的连接方式接入测试计划编排；手动测试作为稳定兜底路径复用现有人工回答录入能力。
...
### Test Theme and Question Service

- 入口：可在 `apps/api/src/modules/brands/` 或独立 `apps/api/src/modules/test-plans/` 中实现。
- 根据 `BrandProfile` 生成测试主题：品牌词、品类词、地域词、人群年龄段、用户痛点、课程或产品、竞品对比、购买决策。
- 根据测试主题生成测试问法候选，标注测试目的、目标平台、推荐优先级和预计测试价值。
- 追光小牛样例作为 deterministic fixture，便于内测闭环验证。
- 覆盖需求 3.1-4.5、17.1-17.2。

### Test Plan Service

- 入口：新增测试计划 controller 和 repository 方法，或扩展 monitoring 模块创建入口。
- 保存首轮测试计划、候选问法选择、目标平台、连接方式、预计耗时和需要用户确认事项。
- 根据平台状态分发到 API 自动测试、浏览器辅助测试或手动测试。
- 支持测试计划模板、复制、编辑和复测。
...
- 平台状态归类为可自动测试、可用浏览器测试、可手动测试、需要配置。
...
- 遇到验证码、登录失效、页面结构变化、平台限制时返回需要用户确认状态，并提供手动测试路径。
...
- AI 监测页：测试问法候选、测试计划、一键测试、测试结果解释。
- 平台连接页：连接状态归类、API 高级设置、浏览器登录引导、手动测试入口。
- 内容生成和任务复测页：增长优化计划、内容任务、发布平台、负责人、截止时间和复测时间。
- 全局用词映射：Prompt 展示为测试问题，优化单元展示为测试主题，平台配置展示为连接 AI 平台。
...
- `OptimizationUnit`: 承载测试主题。
- `UserIntent`: 承载用户意图。
- `BrandPrompt`: 承载测试问法。
- `PlatformConfig`: 承载 API、浏览器、手动和 mock 连接状态。
- `MonitoringRun`: 承载单次测试运行。
...
- Property B3: 任意测试问法候选必须包含测试目的和至少一个目标平台。覆盖需求 4.3-4.4。
- Property B4: 任意浏览器回答成功记录必须关联品牌、平台、测试问题和原始回答。覆盖需求 7.3-7.4。
- Property B5: 任意高风险表达分析结果必须生成“需要你确认”状态和建议改法。覆盖需求 10.4、17.4。
- Property B6: 任意第一版优化计划必须包含负责人、截止时间、发布平台和复测时间。覆盖需求 15.9。
- Property B7: 复测结果必须能关联原始测试计划和优化计划，便于对比优化前后的推荐率、排名和表达准确性。覆盖需求 14.2-15.7。

## Error Handling

- 资料导入失败：返回支持格式、失败原因和手动填写入口。
- 品牌资料缺失：允许继续首轮测试，并标记结果可信度提示。
- AI 平台 API 配置错误：标记平台为需要配置，保留测试计划和手动测试入口。
- 浏览器验证码或风控：停止当前平台自动操作，返回需要用户确认状态和手动测试路径。
- 浏览器登录失效：更新会话状态，提示重新登录。
- 回答匹配失败：提示用户选择对应测试问题或重新粘贴回答。
...
- 测试主题和问法测试覆盖完整资料、缺少城市、缺少竞品、追光小牛样例和属性 B3。
- 测试计划编排测试覆盖 API、浏览器、手动和无可用连接方式。
...
- 前端测试覆盖品牌导入入口、步骤条、问法选择、平台连接状态、手动粘贴、结果解释、优化计划和全局业务化用词。
...
- `当前工作区/geo-platform/packages/shared-types/src/index.ts`
- `当前工作区/geo-platform/apps/api/prisma/schema.prisma`
- `当前工作区/geo-platform/apps/web/src/features/brand-workspace/`
- `当前工作区/geo-platform/apps/web/src/features/monitoring/`
