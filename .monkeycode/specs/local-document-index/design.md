...
本功能新增一款 Windows 10/11 本地桌面应用，暂定产品名为“资料索引”。应用扫描用户明确选择的磁盘或目录，仅采集文件系统元数据，并把分散在不同目录中的同主题文档聚合为版本组。搜索结果以主题为入口，主题详情同时呈现创建时间与最后修改时间两个版本维度。
