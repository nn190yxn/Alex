...
本需求面向品牌方小白用户，把 GEO 平台从“多个页面分别点击生成”升级为“AI 自动化运营员”。系统应自动生成测试问题、自动执行 AI 测试、自动分析结果、自动生成可发布内容、自动按目标平台规则改写，并把关键决策收口到用户确认。第一版聚焦追光小牛内测流程，覆盖豆包、Kimi、DeepSeek、通义千问 API 或浏览器辅助测试，以及知乎、百家号、小红书、公众号、官网 FAQ 等内容发布建议。

## Glossary

- **AI 自动化运营员**: 由系统发起并串联测试、分析、内容生成、平台改写、确认和复测建议的自动化工作流。
- **自动化任务包**: 一次自动化运营流程的结构化记录，包含目标、范围、输入资料、任务状态、确认事项、生成结果和审计摘要。
- **测试问题池**: 系统围绕品牌词、品类词、地域词、人群、痛点、课程、竞品、购买决策、内容缺口和复测目标持续生成的候选问题集合。
- **本轮精选问题**: 系统从测试问题池中筛出的 5 到 6 个高价值问题，用于当前轮次让用户确认并执行测试。
- **确认队列**: 系统需要用户判断的事项集合，例如测试问题确认、平台连接确认、风险表达确认、内容发布确认和复测安排确认。
- **平台规则适配**: 根据目标平台内容特点调整标题、正文结构、语气、标签、长度、合规提醒和发布建议。
- **可发布内容**: 已包含标题、正文、核心卖点、引用依据、平台格式、合规提醒和发布前检查项的内容草稿。
- **发布建议**: 系统基于测试结果、内容类型和平台匹配度给出的建议发布平台、发布顺序和复测时间。
...
**User Story:** AS 品牌负责人, I want 一键启动 AI 自动化运营流程, so that 系统可以主动完成测试、分析和内容生成准备工作。

#### Acceptance Criteria

1. WHEN 用户进入品牌工作区、AI 测试页或增长优化页, the system SHALL 提供“让 AI 帮我跑一轮”入口。
2. WHEN 用户启动 AI 自动化运营员, the system SHALL 创建自动化任务包并展示本次将自动完成的步骤、预计耗时、目标平台和需要用户确认的节点。
3. WHEN 自动化任务包创建后, the system SHALL 读取当前品牌档案、测试主题、平台连接状态、历史测试结果、内容资产和发布记录作为输入。
4. IF 品牌资料不足以启动首轮测试, the system SHALL 生成资料补充问题并允许用户选择继续使用基础资料启动。
5. WHEN 自动化任务包运行中, the system SHALL 展示当前步骤、已完成事项、等待确认事项和失败兜底路径。

### Requirement 2: 自动生成并确认测试问题

**User Story:** AS 品牌负责人, I want 系统直接生成 5 到 6 个测试问题, so that 我只需要确认这些问题是否符合业务。

#### Acceptance Criteria

1. WHEN 自动化任务包进入测试准备步骤, the system SHALL 先生成或更新测试问题池，再从问题池中筛选 5 到 6 个本轮精选问题。
2. WHEN 系统生成测试问题池, the system SHALL 覆盖品牌直问、品类推荐、地域推荐、购买决策、用户痛点、课程需求、竞品对比、内容缺口和复测目标中的高价值场景。
3. WHEN 系统展示本轮精选问题, the system SHALL 标注测试目的、目标平台、推荐理由、预计测试价值和所属测试角度。
4. WHEN 用户确认本轮精选问题, the system SHALL 保存为测试计划并进入自动测试执行步骤。
5. IF 用户调整本轮精选问题, the system SHALL 保存调整后的问题文本、测试目的、目标平台、测试角度和确认记录。
6. IF 系统发现新的品牌资料、测试结果、竞品变化、内容发布记录或复测结果, the system SHALL 更新测试问题池并生成下一轮候选问题。
7. IF 系统无法生成足够本轮精选问题, the system SHALL 使用规则模板补齐候选问题并标记生成依据。

### Requirement 3: 自动执行 AI 测试

**User Story:** AS 品牌运营人员, I want 系统自动向多个 AI 平台提问, so that 我可以快速看到品牌在 AI 回答中的表现。

#### Acceptance Criteria

1. WHEN 测试计划确认后, the system SHALL 按平台连接状态选择 API 测试、浏览器辅助测试或手动测试路径。
2. WHEN 平台具备 API 连接, the system SHALL 自动发送测试问题并保存回答、平台、模型、耗时和调用审计摘要。
3. WHEN 平台具备浏览器连接, the system SHALL 在用户授权会话范围内发送问题、读取回答并保存回答摘要。
4. IF 浏览器连接出现验证码、登录失效、平台限制或页面读取异常, the system SHALL 将该平台加入确认队列并提供手动测试入口。
5. IF 平台需要手动测试, the system SHALL 生成可复制问题和粘贴回答入口，并保持同一套分析流程。
6. WHEN 自动测试完成, the system SHALL 触发回答分析并更新自动化任务包状态。
...
**User Story:** AS 品牌运营人员, I want 系统根据测试结果自动写推文和内容草稿, so that 我可以直接审核和发布。
...
3. WHEN 用户确认测试问题、分析判断、内容草稿或平台改写结果, the system SHALL 记录确认人、确认时间、确认结论和后续动作。
...
1. WHEN 发布建议确认后, the system SHALL 生成复测计划建议并关联原测试计划、内容版本和发布记录。
2. WHEN 复测时间到达或用户手动触发复测, the system SHALL 使用相同测试问题和平台执行可比测试。
3. WHEN 复测完成, the system SHALL 对比推荐率、排名、表达准确性、引用来源和竞品压制变化。
4. IF 复测结果未达到改善目标, the system SHALL 生成下一轮内容建议和测试问题建议。
...
1. 第一版自动化任务从“持续生成测试问题池，并为当前轮次精选 5 到 6 个测试问题”开始，串联测试、分析、内容生成、平台改写、确认和复测建议。
2. 第一版平台测试复用现有 API、浏览器辅助和手动测试路径。
3. 第一版内容平台适配覆盖知乎、百家号、小红书、公众号和官网 FAQ。
4. 第一版用户操作重点是确认测试问题、确认风险判断、确认内容和确认发布建议。
