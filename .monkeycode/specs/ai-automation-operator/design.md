...
AI 自动化运营员在现有 GEO 平台能力之上增加一层任务编排。现有系统已经具备品牌档案、测试问题生成、测试计划执行、LLM 回答分析、增长优化计划、内容生成、发布中心和复测任务；本设计新增自动化任务包、确认队列和平台规则适配，让这些能力从分散按钮操作升级为串联流程。

第一版实现目标是让品牌方点击一次入口后，系统持续维护测试问题池，并从问题池中为当前轮次精选 5 到 6 个问题让用户确认。用户确认后，系统自动测试、自动分析结果、自动生成内容、按目标平台改写，并把发布建议和复测建议收口到确认队列。
...
graph TD
    ...
    B --> C["QuestionGeneration 生成测试问题"]
    C --> D["QuestionPool 筛选本轮问题"]
    D --> E["ConfirmationQueue 确认测试问题"]
    E --> F["TestPlanExecution 执行 AI 测试"]
...
新增编排层只负责步骤状态、输入收集、确认节点和错误转移。具体业务能力继续复用现有模块：测试问题复用 `TestQuestionService` 和 `LLMOrchestrationService.question_generation`，测试执行复用测试计划执行接口，回答分析复用 `answer_analysis` 和规则二次校验，内容生成复用 `ContentGenerationWorker`，发布和复测复用已有发布中心与任务复测能力。
...
- `prepareQuestions(packageId)`：更新测试问题池，筛选 5 到 6 个本轮精选问题并创建确认事项。
- `continueAfterConfirmation(packageId)`：根据确认结果推进下一步。
- `runTestPlan(packageId)`：创建或复用测试计划并执行。
...
职责：维护持续增长的测试问题池，并按当前运营目标筛选本轮精选问题。

输入来源：品牌档案、测试主题、历史测试问题、AI 回答分析、竞品变化、内容缺口、发布记录和复测结果。

核心规则：

- 问题池可以持续扩展，当前轮次只展示 5 到 6 个精选问题。
- 同一轮精选问题应覆盖不同测试角度，避免重复测试同一个意图。
...
2. 测试问题池可以持续增长，本轮精选问题数量默认控制在 5 到 6 个。
3. 自动化任务包进入下一步骤前，所有必需确认事项必须处于已处理状态。
4. 自动测试失败的平台必须保留可恢复路径：浏览器确认或手动测试。
...
- 持续生成测试问题池，并从问题池中精选 5 到 6 个问题保存为测试计划。
- 测试计划确认后执行 API、浏览器和手动兜底路径。
- 分析结果驱动内容生成和平台改写。
- 内容确认后创建发布记录和复测建议。
