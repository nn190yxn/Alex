...
- Instructions:
  ...
  - `tauri.conf.json` 的 `bundle.targets` 只接受 `deb`/`rpm`/`appimage`/`msi`/`nsis`/`app`/`dmg`；Windows 上的 WiX 产物对应 `msi`，写成 `wix` 会让构建脚本以 `data did not match any variant of untagged enum BundleTargetInner` 失败。
  ...
  - `tauri-build` 当前最新为 2.6.x，版本约束写 `2.8` 会解析失败；tauri 与 tauri-build 统一用 `2` 这样的主版本约束。
