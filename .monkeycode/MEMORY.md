...
[跨 Agent 协作：先产出可执行蓝图再由我复审]
- Date: 2026-05-04
- Context: 用户在要求优化“工具与知识库联通效率”后明确新的协作方式
- Instructions:
  - 先由我产出一份详细、可落地、可执行的实施蓝图文档。
  - 文档需要按“给另一位 Agent 的任务说明+步骤+验收标准”来写。
  - 另一位 Agent 按文档执行改造后，再由我做最终审核。
  - 文档路径必须清晰可定位，确保另一位 Agent 能直接找到。
...
[所有思考过程和回复必须使用中文]
- Date: 2026-05-04
- Context: 用户多次强调后再次明确要求
- Instructions:
  - 所有思考过程、工具调用标题、回复内容、代码注释说明都必须使用中文。
  - 这是用户在多次沟通中反复强调的强制要求，不是可选项。
  - 代码中的变量名、函数名可以保持英文，但注释和解释必须用中文。
  - 即使执行英文指令，思考和回复也应该用中文。
